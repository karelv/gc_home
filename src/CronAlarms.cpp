...
void CronEventClass::updateNextTrigger(bool forced, time_t timenow)
{
  if (isEnabled) {
...
static time_t get_time_default()
{
  return time(nullptr);
}
...
CronClass::CronClass()
{
  ...
  this->get_time = get_time_default;
}


void CronClass::setGetTimeFunction(time_t (*get_time)(void))
{
...
void CronClass::globalUpdateNextTrigger()
{
  for (uint8_t eachCronId = 0; eachCronId < dtNBR_ALARMS; eachCronId++) {
    if (Alarm[eachCronId].isEnabled) {
      Alarm[eachCronId].updateNextTrigger(true, this->get_time());
    }
  }
}
...
void CronClass::enable(CronID_t ID)
{
  if (isAllocated(ID)) {
    ...
    Alarm[ID].updateNextTrigger(false, this->get_time());
  }
}
...
void CronClass::serviceAlarms()
{
  if (globalEnabled && !isServicing) {
    isServicing = true;
    time_t timenow = this->get_time();

    for (servicedCronId = 0; servicedCronId < dtNBR_ALARMS; servicedCronId++) {
      if (Alarm[servicedCronId].isEnabled && (timenow >= Alarm[servicedCronId].nextTrigger)) {
        ...
        if (Alarm[servicedCronId].isOneShot) {
          ...
          Alarm[servicedCronId].updateNextTrigger(false, timenow);
        }
        if (TickHandler != NULL) {
          (*TickHandler)(Alarm[servicedCronId].data);     // call the handler
        }
      }
    }
    isServicing = false;
...
CronID_t CronClass::create(const char * cronstring, OnTick_t onTickHandler, bool isOneShot, void *data)
{
  for (uint8_t id = 0; id < dtNBR_ALARMS; id++) {
    if (!isAllocated(id)) {
